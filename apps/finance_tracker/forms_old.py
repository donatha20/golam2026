...
class ExpenditureForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Use Settings-managed expense categories as expenditure types when available, otherwise fall back to model defaults.
        configured_types = list(
            ExpenseCategory.objects.filter(is_active=True).order_by('name').values_list('code', 'name')
        )
        
        if configured_types:
            # Use configured expense categories from database
            self.fields['expenditure_type'].choices = configured_types
        else:
            # Fall back to the model's default choices
            self.fields['expenditure_type'].choices = Expenditure.EXPENDITURE_TYPES

        # Keep existing type selectable for edits even if now inactive.
        current_type = getattr(self.instance, 'expenditure_type', None)
        if current_type:
            current_choices_dict = dict(self.fields['expenditure_type'].choices)
            if current_type not in current_choices_dict:
